...
public class OrangePortalParticleTask extends BukkitRunnable {
    @Override
    public void run() {
    ...
    private void generateParticle(Portal portal) {
        ...
        if (portal.getType() == PortalType.ORANGE) {
            if (portal.getFacing().equals("NS")) {
                ...
                for (double theta = 0; theta < 360; theta += 5) {
                    ...
                    world.spawnParticle(Particle.DUST_COLOR_TRANSITION, location.x() + deltaX, location.y() + deltaY, location.getZ(), 0, new Particle.DustTransition(Color.ORANGE, Color.RED, 0.5f));
                }
                ...
                for (double theta = 0; theta < 360; theta += 5) {
                    ...
                    deltaY = b * Math.sin(theta);
                    world.spawnParticle(Particle.DUST_COLOR_TRANSITION, location.x(), location.y() + deltaY, location.getZ() + deltaZ, 0, new Particle.DustTransition(Color.ORANGE, Color.RED, 0.5f));
                }
            }
        }
    }
